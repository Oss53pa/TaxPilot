...
const ModernFiscalCalendar: React.FC = () => {
  ...
  const calculatePenalty = (deadline: FiscalDeadline): number => {
    ...
    const basePenalty = (deadline.amount * deadline.penaltyRate) / 100
    const dailyPenalty = basePenalty / 30 // Pénalité journalière
    return Math.min(basePenalty + (dailyPenalty * daysOverdue), deadline.amount * 0.5) // Plafond à 50%
  }
                    ...
                    {getCalendarDays().map((day, index) => (
                      <Grid item xs={12 / 7} key={index}>
                        ...
                        >
                          ...
                          {day.deadlines.map((deadline, idx) => (
                            <Box
                              key={deadline.id}
                              sx={{
                                mt: 0.5,
                              ...
                              {deadline.amount ? (
                                <Box>
                                  <Typography variant="body2" sx={{ fontWeight: 600 }}>
                                    {deadline.amount.toLocaleString()} FCFA
                                  </Typography>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <Paper sx={{ p: 2, backgroundColor: alpha(theme.palette.primary.main, 0.02) }}>
                ...
                <Typography variant="body2">
                  Pénalité de base: 100 000 FCFA (10% du principal)
                </Typography>
                <Typography variant="body2">
                  Pénalité journalière: 3 333 FCFA
                </Typography>
                <Typography variant="body2" sx={{ fontWeight: 600, mt: 1 }}>
                  Total pénalités: 200 000 FCFA
                </Typography>
              </Paper>
            </Grid>
          </Grid>

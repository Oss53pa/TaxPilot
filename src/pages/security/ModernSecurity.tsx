...
interface SecurityPolicy {
  id: string;
  name: string;
  type: 'password' | 'session' | 'mfa' | 'access' | 'data';
  rules: PolicyRule[];
  isActive: boolean;
  priority: number;
  appliesTo: 'all' | 'role' | 'group' | 'user';
  targets?: string[];
...
interface PolicyRule {
  ...
  condition: string;
  value: any;
  action: 'allow' | 'deny' | 'require' | 'notify';
  message?: string;
}
...
const ModernSecurity: React.FC = () => {
    ...
    const mockPolicies: SecurityPolicy[] = [
      {
        ...
        rules: [
          ...
          },
          {
            ...
            value: 'high',
            action: 'require',
            message: 'Le mot de passe doit contenir majuscules, minuscules, chiffres et symboles'
          },
          {
            id: 'rule-3',
            condition: 'expiry_days',
            value: 90,
            action: 'require',
            message: 'Le mot de passe expire tous les 90 jours'
          }
        ],
        isActive: true,
        priority: 1,
        ...
        rules: [
          {
            ...
            value: 50,
            action: 'require',
            message: 'MFA obligatoire pour les rôles sensibles'
          }
        ],
        isActive: true,
        priority: 2,
        appliesTo: 'role',
        targets: ['role-1', 'role-3'],
        enforcement: 'strict',
        createdAt: new Date('2024-01-15'),
  ...
  const getSecurityScoreColor = () => {
    ...
    if (securityScore >= 60) return '#ff9800';
    if (securityScore >= 40) return '#f57c00';
    return '#f44336';
  };

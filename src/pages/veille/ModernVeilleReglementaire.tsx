...
const ModernVeilleReglementaire: React.FC = () => {
  ...
  const [notifications, setNotifications] = useState<RegulationNotification[]>([]);
  ...
  // EX-VEILLE-003: Analyse d'impact automatique
  const analyzeImpact = (regulation: Regulation) => {
    const impacts: Impact[] = [];
    
    // Analyse basée sur les mots-clés et catégories
    if (regulation.category === 'fiscal') {
      impacts.push({
        id: `impact-auto-${Date.now()}`,
        area: 'Comptabilité et Fiscalité',
        description: 'Révision nécessaire des procédures fiscales',
        severity: regulation.priority === 'critical' ? 'high' : 'medium',
        actionRequired: 'Analyser et mettre à jour les processus',
        responsibleDepartment: 'Finance',
        status: 'pending'
      });
    }
  ...
  const filteredRegulations = regulations.filter(regulation => {
    ...
    const matchesCategory = filterCategory === 'all' || regulation.category === filterCategory;
    const matchesJurisdiction = filterJurisdiction === 'all' || regulation.jurisdiction === filterJurisdiction;
    const matchesStatus = filterStatus === 'all' || regulation.complianceStatus === filterStatus;
    
    return matchesSearch && matchesCategory && matchesJurisdiction && matchesStatus;
  });

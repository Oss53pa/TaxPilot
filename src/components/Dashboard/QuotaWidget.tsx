  ...
  const getAlertMessage = () => {
    ...
    if (quotaPercentage >= 90) {
      return {
        ...
        message: `Attention ! Il vous reste ${quotaRemaining} liasse${quotaRemaining > 1 ? 's' : ''} sur ${organization.liasses_quota}. Pensez à upgrader votre plan.`,
      }
    }
    ...
    if (quotaPercentage >= 70) {
      return {
        ...
        message: `Vous avez utilisé ${quotaPercentage}% de votre quota. ${quotaRemaining} liasse${quotaRemaining > 1 ? 's' : ''} restante${quotaRemaining > 1 ? 's' : ''}.`,
      }
    }
          ...
          <Box>
            ...
            {!isUnlimited && (
              <>
                ...
                />
                <Typography variant="caption" color="text.secondary">
                  {quotaPercentage}% utilisé • {quotaRemaining} restante{quotaRemaining > 1 ? 's' : ''}
                </Typography>
              </>
            )}
          </Box>
          ...
          >
            <Box>
              <Typography variant="subtitle2" fontWeight={600}>
                Plan {organization.get_subscription_plan_display || organization.subscription_plan}
              </Typography>
              <Typography variant="caption" color="text.secondary">
          ...
          <Stack direction="row" spacing={2}>
            <Box flex={1}>
              ...
              <Typography variant="body2" fontWeight="bold">
                {organization.members?.length || 0} / {organization.users_quota}
              </Typography>
            </Box>
            <Box flex={1}>
              <Typography variant="caption" color="text.secondary">
              ...
              <Typography variant="body2" fontWeight="bold">
                {organization.storage_quota_gb} GB
              </Typography>
            </Box>
          </Stack>
        </Stack>

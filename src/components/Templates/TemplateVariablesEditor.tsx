...
export default function TemplateVariablesEditor() {
  ...
  const loadTemplateDetails = async () => {
    ...
    try {
      ...
      const varsData = await templatesService.getTemplateVariables(selectedTemplate.id)
      setVariables(varsData.results || varsData)

      // Charger les sections
      const sectionsData = await templatesService.getTemplateSections(selectedTemplate.id)
  ...
  const addSection = async () => {
    ...
    try {
      const created = await templatesService.addTemplateSection(
        selectedTemplate.id,
        newSection
      )
      ...
      toast({
        ...
        description: `La section "${created.nom}" a été créée`
      })
  ...
  const updateSection = async (sectionId: string, updates: Partial<TemplateSection>) => {
    if (!selectedTemplate) return

    try {
      await templatesService.updateTemplateSection(
        selectedTemplate.id,
        sectionId,
        updates
      )

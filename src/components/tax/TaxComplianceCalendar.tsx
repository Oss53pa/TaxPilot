...
import { format, addMonths, subMonths, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, isSameDay, isToday, isPast, isFuture } from 'date-fns'
...
export default function TaxComplianceCalendar() {
  ...
  const loadObligations = async () => {
    try {
      ...
      const echeances = await taxService.getObligationsEcheances(entrepriseId, 30)
  ...
  const exportCalendar = async () => {
    try {
      ...
      const data = await taxService.getObligationsCalendar({
        entreprise: entrepriseId,
        annee,
  ...
  const getDayClass = (date: Date) => {
    ...
    if (hasRetard) return 'bg-red-100 text-red-900 font-bold'
    if (hasAFaire && isPast(date)) return 'bg-orange-100 text-orange-900 font-bold'
    if (hasAFaire) return 'bg-blue-100 text-blue-900 font-bold'
            ...
            <div className="space-y-4 py-4">
              ...
              {selectedObligation.montant_estimé && (
                <div className="p-3 bg-blue-50 rounded-lg">
                  ...
                  <p className="text-xl font-bold text-blue-600">
                    {selectedObligation.montant_estimé.toLocaleString('fr-FR')} FCFA
                  </p>
                </div>
              )}
              ...
              {selectedObligation.penalites_retard && selectedObligation.penalites_retard > 0 && (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    Pénalités de retard: {selectedObligation.penalites_retard.toLocaleString('fr-FR')} FCFA
                  </AlertDescription>
                </Alert>
              )}

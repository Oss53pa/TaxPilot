...
const CalculTVADialog: React.FC<CalculTVADialogProps> = ({
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const calculResult = await taxService.calculateTVA(data)
              ...
              <CardContent>
                <Grid container spacing={2} alignItems="center">
                  <Grid item xs={12} md={6}>
                    ...
                    >
                      {formatMontant(result.montant_a_payer > 0 ? result.montant_a_payer : result.credit_reporter)} FCFA
                    </Typography>
                  </Grid>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <Typography variant="caption" sx={{ fontWeight: 600, fontFamily: 'monospace' }}>
                          {formatMontant(result.tva_collectee)} FCFA
                        </Typography>
                      </Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Typography variant="caption">TVA Déductible:</Typography>
                        <Typography variant="caption" sx={{ fontWeight: 600, fontFamily: 'monospace' }}>
                          - {formatMontant(result.tva_deductible)} FCFA
                        </Typography>
                      </Box>
                      {result.credit_precedent > 0 && (
                        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                          ...
                          <Typography variant="caption" sx={{ fontWeight: 600, fontFamily: 'monospace' }}>
                            - {formatMontant(result.credit_precedent)} FCFA
                          </Typography>
                        </Box>
                      )}
                    </Box>
                ...
                <List dense>
                  {result.details_calcul.map((detail, index) => (
                    <ListItem key={index} disableGutters>
                      ...
                      >
                        {formatMontant(detail.montant)} FCFA
                      </Typography>
                    </ListItem>
                  ))}
                </List>

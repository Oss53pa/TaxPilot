...
const TablesCalculImpots: React.FC = () => {
  ...
  const [parametresFiscaux, setParametresFiscaux] = useState({
    taux_is: 30, // Impôt sur les sociétés (30% standard OHADA)
    taux_tva_standard: 19.25, // TVA standard (ex: Cameroun)
    taux_tva_reduit: 5.5,
  ...
  const [donneesComptables, setDonneesComptables] = useState({
  ...
  const calculerImpotSociete = () => {
    ...
    const reports_utilisables = Math.min(reports_deficitaires, resultat_fiscal * 0.5)
    const base_imposable = Math.max(0, resultat_fiscal - reports_utilisables)
    
    // Calcul IS
    const is_calcule = base_imposable * (taux_is / 100)
  ...
  const calculerTaxesTVA = () => {
    ...
    const { taux_tva_standard, regime_fiscal } = parametresFiscaux
    
    // TVA collectée
    const tva_collectee = chiffre_affaires_ht * (taux_tva_standard / 100)
    
    // TVA déductible
    const tva_deductible = achats_ht * (taux_tva_standard / 100)
    ...
    const taxes_diverses = [
      {
        ...
        base_imposable: chiffre_affaires_ht,
        taux: 1, // 1% exemple
        montant_calcule: chiffre_affaires_ht * 0.01,
        montant_declare: chiffre_affaires_ht * 0.01,
        ecart: 0
      },
      {
        code: 'TAX_002', 

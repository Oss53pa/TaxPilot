...
const Note17SYSCOHADA: React.FC = () => {
  ...
  const repartitionSecteur = SECTEURS_ACTIVITE.map(secteur => {
    const donnees = chiffresAffaires.filter(ca => ca.secteur === secteur)
    const montant = donnees.reduce((sum, ca) => sum + ca.montantN, 0)
    const montantN1 = donnees.reduce((sum, ca) => sum + ca.montantN1, 0)
    const evolution = montantN1 > 0 ? ((montant - montantN1) / montantN1) * 100 : 0
  ...
  }).filter(secteur => secteur.montant > 0)
            ...
            <Card variant="outlined">
              <CardContent sx={{ textAlign: 'center' }}>
                ...
                <Stack direction="row" justifyContent="center" alignItems="center" spacing={1}>
                  {totaux.evolution > 0 ? <GrowthIcon color="success" /> : <DeclineIcon color="error" />}
                  <Typography 
                    variant="h6" 
                    color={totaux.evolution > 0 ? 'success.main' : 'error.main'}
                    sx={{ fontWeight: 600 }}
                  >
                    {totaux.evolution > 0 ? '+' : ''}{formatPercent(totaux.evolution)}
                  </Typography>
                </Stack>
          ...
          <Table size="small">
            ...
            <TableBody>
              {chiffresAffaires.map(ca => {
                const zone = ZONES_GEOGRAPHIQUES.find(z => z.id === ca.zone)
                const ZoneIcon = zone?.icon || DomesticIcon
              ...
              <TableRow sx={{ backgroundColor: alpha(theme.palette.primary.main, 0.1), fontWeight: 600 }}>
                ...
                <TableCell align="right" sx={{ fontWeight: 600 }}>
                  <Stack direction="row" alignItems="center" justifyContent="flex-end" spacing={0.5}>
                    {totaux.evolution > 0 ? <GrowthIcon fontSize="small" color="success" /> : <DeclineIcon fontSize="small" color="error" />}
                    <Typography 
                      variant="body2" 
                      color={totaux.evolution > 0 ? 'success.main' : 'error.main'}
                      sx={{ fontWeight: 600 }}
      ...
      <CardContent>
        ...
        <TableContainer>
          <Table size="small">
            ...
            <TableBody>
              {evolutionMensuelle.map((periode, index) => (
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card sx={{ textAlign: 'center', backgroundColor: alpha(theme.palette.success.main, 0.1) }}>
              <CardContent sx={{ py: 2 }}>
                <Stack direction="row" justifyContent="center" alignItems="center" spacing={1}>
                  {totaux.evolution > 0 ? <GrowthIcon color="success" /> : <DeclineIcon color="error" />}
                  <Typography variant="h6" color={totaux.evolution > 0 ? "success.main" : "error.main"} sx={{ fontWeight: 600 }}>
                    {totaux.evolution > 0 ? '+' : ''}{formatPercent(totaux.evolution)}
                  </Typography>
                </Stack>

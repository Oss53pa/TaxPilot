...
const CATEGORIES_CHARGES = [
  ...
  { id: 'charges_sociales', label: 'Charges sociales', icon: SocialIcon, color: '#e74c3c' },
...
const Note19SYSCOHADA: React.FC = () => {
  ...
  const handleSave = () => {
    console.log('Sauvegarde Note 19:', { charges, effectifs, comment })
    setHasChanges(false)
  }
      ...
      <CardContent>
        ...
        <TableContainer>
          <Table size="small">
            ...
            <TableBody>
              {charges.map(charge => {
                const catInfo = CATEGORIES_CHARGES.find(c => c.id === charge.categorie)
                const IconComponent = catInfo?.icon || PersonnelIcon
              ...
              <TableRow sx={{ backgroundColor: alpha(theme.palette.primary.main, 0.1), fontWeight: 600 }}>
                ...
                <TableCell align="right" sx={{ fontWeight: 600 }}>
                  <Stack direction="row" alignItems="center" justifyContent="flex-end" spacing={0.5}>
                    {totaux.evolution > 0 ? <GrowthIcon fontSize="small" color="error" /> : totaux.evolution < 0 ? <DeclineIcon fontSize="small" color="success" /> : null}
                    <Typography 
                      variant="body2" 
                      color={totaux.evolution > 0 ? 'error.main' : 'success.main'}
                      sx={{ fontWeight: 600 }}
                    >
                ...
                <TableCell align="right" sx={{ fontWeight: 600 }}>
                  {formatPercent(totaux.pourcentageCA)}
                </TableCell>
              ...
              <Stack spacing={1}>
                ...
                <Box>
                  <Stack direction="row" justifyContent="space-between">
                    ...
                    <Typography variant="body2" sx={{ fontWeight: 600, color: totaux.evolution > 7.3 ? theme.palette.error.main : theme.palette.success.main }}>
                      {formatPercent(totaux.evolution)} vs 7.3%
                    </Typography>
                  </Stack>
                </Box>
              </Stack>
            ...
            <Card sx={{ textAlign: 'center', backgroundColor: alpha(theme.palette.success.main, 0.1) }}>
              <CardContent sx={{ py: 2 }}>
                <Stack direction="row" justifyContent="center" alignItems="center" spacing={1}>
                  {totaux.evolution > 0 ? <GrowthIcon color="error" /> : <DeclineIcon color="success" />}
                  <Typography variant="h6" color={totaux.evolution > 0 ? "error.main" : "success.main"} sx={{ fontWeight: 600 }}>
                    {totaux.evolution > 0 ? '+' : ''}{formatPercent(totaux.evolution)}
                  </Typography>
                </Stack>

...
const AuditTrailsView: React.FC = () => {
  ...
  const loadAuditLogs = async () => {
    try {
      ...
      const response = await apiClient.get('/api/v1/audit/audit-logs/', params)
  ...
  const loadStatistics = async () => {
    try {
      const response = await apiClient.get('/api/v1/audit/audit-logs/statistics/')
      setStats({
  ...
  const handleVerifyChain = async () => {
    try {
      ...
      const response = await apiClient.get('/api/v1/audit/audit-logs/verify_chain/')
  ...
  const handleVerifyEntry = async (entry: AuditLogEntry) => {
    try {
      const response = await apiClient.get(`/api/v1/audit/audit-logs/${entry.id}/verify_entry/`)

...
const AuditHistory: React.FC = () => {
  ...
  const [sessionAnomalies, setSessionAnomalies] = useState<AuditAnomalie[]>(null)
  ...
  const loadSessions = async () => {
    ...
    try {
      ...
      const response = await apiClient.get('/api/v1/audit/sessions/', { params })
  ...
  const loadStatistics = async () => {
    try {
      ...
      const response = await apiClient.get('/api/v1/audit/statistics/', { params })
  ...
  const handleViewDetails = async (session: AuditSession) => {
    ...
    setSessionAnomalies(null)

    try {
      const response = await apiClient.get(`/api/v1/audit/sessions/${session.id}/anomalies/`)

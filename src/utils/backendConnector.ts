...
export const replaceMockData = async (
  serviceType: keyof typeof SERVICE_MAPPINGS,
  method: string,
  params?: any,
  defaultData?: any
) => {
  try {
    ...
    const response = await mapping.service[methodName](params)
...
export const useBackendService = (serviceType: keyof typeof SERVICE_MAPPINGS) => {
  const mapping = SERVICE_MAPPINGS[serviceType]

...
logger.debug('')
logger.debug('═══════════════════════════════════════════════════════════════════════════════════')
logger.debug('              RAPPORT DE VÉRIFICATION - LIASSE FISCALE SYSCOHADA                  ')
logger.debug('═══════════════════════════════════════════════════════════════════════════════════')
logger.debug('')

// Detail tables
logger.debug('── BILAN ACTIF (extraits) ──')
logger.debug(`  AE (Brevets/logiciels) : brut=${fmt(actifByRef['AE']?.brut||0)}, amort=${fmt(actifByRef['AE']?.amortProv||0)}, net=${fmt(actifByRef['AE']?.net||0)}`)
logger.debug(`  AK (Bâtiments)         : brut=${fmt(actifByRef['AK']?.brut||0)}, amort=${fmt(actifByRef['AK']?.amortProv||0)}, net=${fmt(actifByRef['AK']?.net||0)}`)
logger.debug(`  AM (Matériel)          : brut=${fmt(actifByRef['AM']?.brut||0)}, amort=${fmt(actifByRef['AM']?.amortProv||0)}, net=${fmt(actifByRef['AM']?.net||0)}`)
logger.debug(`  BJ (Clients)           : brut=${fmt(actifByRef['BJ']?.brut||0)}, net=${fmt(actifByRef['BJ']?.net||0)}`)
logger.debug(`  BS (Trésorerie)        : brut=${fmt(actifByRef['BS']?.brut||0)}, net=${fmt(actifByRef['BS']?.net||0)}`)
logger.debug(`  TOTAL ACTIF NET        : ${fmt(totalActifNet)}`)
logger.debug('')

logger.debug('── BILAN PASSIF (extraits) ──')
logger.debug(`  CA (Capital)           : ${fmt(passifByRef['CA']?.montant||0)}`)
logger.debug(`  CD (Réserves)          : ${fmt(passifByRef['CD']?.montant||0)}`)
logger.debug(`  CH (Résultat net)      : ${fmt(passifByRef['CH']?.montant||0)}`)
logger.debug(`  DB (Emprunts)          : ${fmt(passifByRef['DB']?.montant||0)}`)
logger.debug(`  DJ (Fournisseurs)      : ${fmt(passifByRef['DJ']?.montant||0)}`)
logger.debug(`  DL (Autres dettes)     : ${fmt(passifByRef['DL']?.montant||0)}`)
logger.debug(`  TOTAL PASSIF           : ${fmt(totalPassif)}`)
logger.debug('')

logger.debug('── COMPTE DE RÉSULTAT ──')
logger.debug(`  Total produits         : ${fmt(totalProduits)}`)
logger.debug(`  Total charges          : ${fmt(totalCharges)}`)
logger.debug(`  Résultat net (XI)      : ${fmt(resultatCdR)}`)
logger.debug('')

logger.debug('── SIG (cascade) ──')
logger.debug(`  SIG1 Marge commerciale : ${fmt(sig.SIG1)}`)
logger.debug(`  SIG2 Production        : ${fmt(sig.SIG2)}`)
logger.debug(`  SIG3 Valeur ajoutée    : ${fmt(sig.SIG3)}`)
logger.debug(`  SIG4 EBE               : ${fmt(sig.SIG4)}`)
logger.debug(`  SIG5 Rés. exploitation : ${fmt(sig.SIG5)}`)
logger.debug(`  SIG6 Rés. financier    : ${fmt(sig.SIG6)}`)
logger.debug(`  SIG7 RAO               : ${fmt(sig.SIG7)}`)
logger.debug(`  SIG8 Rés. HAO          : ${fmt(sig.SIG8)}`)
logger.debug(`  SIG9 Résultat net      : ${fmt(sig.SIG9)}`)
logger.debug('')

logger.debug('── TFT ──')
logger.debug(`  FA  Résultat net       : ${fmt(tft.FA)}`)
logger.debug(`  FB  Dotations          : ${fmt(tft.FB)}`)
logger.debug(`  FC  Reprises           : ${fmt(tft.FC)}`)
logger.debug(`  FD  Plus-values        : ${fmt(tft.FD)}`)
logger.debug(`  FE  CAFG               : ${fmt(tft.FE)}`)
logger.debug(`  FS  Trésorerie nette   : ${fmt(tft.FS)}`)
logger.debug('')

logger.debug('═══════════════════════════════════════════════════════════════════════════════════')
logger.debug('                          VÉRIFICATIONS AUTOMATIQUES                               ')
logger.debug('═══════════════════════════════════════════════════════════════════════════════════')
logger.debug('')
logger.debug('┌────────┬───────────────────────────────────────────┬──────────────────┬──────────────────┬────────┐')
logger.debug('│ ID     │ Vérification                              │ Attendu          │ Obtenu           │ Statut │')
logger.debug('├────────┼───────────────────────────────────────────┼──────────────────┼──────────────────┼────────┤')
...
for (const r of results) {
  ...
  logger.debug(`│ ${id} │ ${label} │ ${exp} │ ${act} │${st}│`)
}
...
logger.debug(`│ TOTAL  │ ${summaryLabel.padEnd(41)} │                  │                  │${pct === 100 ? ' 100% ' : ' FAIL '}│`)
logger.debug('└────────┴───────────────────────────────────────────┴──────────────────┴──────────────────┴────────┘')
logger.debug('')

if (pct === 100) {
  logger.debug('*** TOUTES LES VÉRIFICATIONS SONT PASSÉES — 100% ***')
} else {
  logger.debug(`*** ${total - passed} VÉRIFICATION(S) EN ÉCHEC ***`)
  for (const r of results.filter(r => !r.pass)) {
    logger.debug(`  FAIL: ${r.id} ${r.label} — attendu ${fmt(r.expected)}, obtenu ${fmt(r.actual)}`)
  }
}
logger.debug('')

...
const Audit: React.FC = () => {
  ...
  const [_auditSessions, setAuditSessions] = useState<any[]>([])
  ...
  const loadAuditData = async () => {
    try {
      ...
      const sessionsResponse = sessionsRaw as Record<string, any>
      const entreprisesResponse = entreprisesRaw as Record<string, any>
      ...
      if (sessionsResponse.results?.[0]) {
        const anomaliesResponse = await auditService.getAuditAnomalies(sessionsResponse.results[0].id) as Record<string, any>

...
const ModernLiasse: React.FC = () => {
  ...
  const [_typeLiasse, setTypeLiasse] = useState<TypeLiasse>('SN')

  // Detect type from entreprise or URL params
  useEffect(() => {
    try {
      const userStr = localStorage.getItem('fiscasync_user')
      if (userStr) {
        const user = JSON.parse(userStr)
        const entreprise = user?.entreprise_courante
        if (entreprise?.type_liasse_recommande) {
          const type = entreprise.type_liasse_recommande as TypeLiasse
          setTypeLiasse(type)
          // Set initial section based on type
          if (type === 'SMT') setActiveSection('bilan-smt')
          else if (type === 'BANQUE') setActiveSection('bilan-banque')
          else if (type === 'ASSURANCE') setActiveSection('bilan-assurance')
          else if (type === 'MICROFINANCE') setActiveSection('bilan-microfinance')
          else if (type === 'EBNL') setActiveSection('bilan-ebnl')
        }
      }
    } catch { /* use default SN */ }
  }, [])

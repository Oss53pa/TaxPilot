...
import type { ControleDefinition } from '@/types/audit.types'
import { registerLevel0Controls } from '@/services/audit/controls/level0-structural'
import { registerLevel1Controls } from '@/services/audit/controls/level1-fundamental'
import { registerLevel2Controls } from '@/services/audit/controls/level2-ohada-conformity'
import { registerLevel3Controls } from '@/services/audit/controls/level3-balance-sense'
import { registerLevel4Controls } from '@/services/audit/controls/level4-inter-account'
import { registerLevel5Controls } from '@/services/audit/controls/level5-year-over-year'
...
interface AlgorithmIA {
  ...
  type: 'DETECTION' | 'CLASSIFICATION' | 'PREDICTION' | 'COHERENCE'
...
const ControlPointsManager: React.FC = () => {
  ...
  const [registryControls, setRegistryControls] = useState<ControleDefinition[]>([])
  const [registryFilter, setRegistryFilter] = useState<number | -1>(-1)
  ...
  const getPorteeColor = (portee: string) => {
    switch (portee) {
      case 'BALANCE': return { bgcolor: 'info.50', color: 'primary.main' } // Bleu
      case 'LIASSE': return { bgcolor: 'success.50', color: 'success.main' }  // Vert
      case 'BOTH': return { bgcolor: 'warning.50', color: 'warning.main' }    // Orange
      default: return { bgcolor: 'grey.50', color: 'text.secondary' }        // Gris
    }
  }
  ...
  useEffect(() => {
    ...
    setAlgorithmes([
      ...
      },
      {
        ...
        parametres: {
          taux_tva_standard: { type: 'number', default: 0.18, description: 'Taux TVA standard 18% Côte d\'Ivoire — CGI Art. 356' },
...
4. DIAGNOSTIC:
  ...
  // Charger les 108 controles du moteur d'audit
  useEffect(() => {
    if (controlRegistry.size === 0) {
    ...
    // Restaurer les etats actif/inactif depuis localStorage
    const defs = controlRegistry.getAllDefinitions()
    for (const d of defs) {
      const stored = localStorage.getItem(`fiscasync_audit_control_${d.ref}`)
      if (stored !== null) {
        controlRegistry.setActive(d.ref, JSON.parse(stored))
      }
    }
    setRegistryControls([...controlRegistry.getAllDefinitions()])
  }, [])

  const handleToggleRegistryControl = (ref: string) => {
    const ctrl = controlRegistry.get(ref)
    if (ctrl) {
      controlRegistry.setActive(ref, !ctrl.definition.actif)
      setRegistryControls([...controlRegistry.getAllDefinitions()])
      // Persister dans localStorage
      const key = `fiscasync_audit_control_${ref}`
      localStorage.setItem(key, JSON.stringify(!ctrl.definition.actif))
    }
  }

  const filteredRegistryControls = registryFilter === -1
    ? registryControls
    : registryControls.filter((c) => c.niveau === registryFilter)
  ...
  const severiteColor = (sev: string) => {
    switch (sev) {
      case 'BLOQUANT': return { bgcolor: 'error.50', color: 'error.main' }
      case 'MAJEUR': return { bgcolor: 'warning.50', color: 'warning.main' }
      case 'MINEUR': return { bgcolor: '#fefce8', color: 'warning.main' }
      case 'INFO': return { bgcolor: 'info.50', color: 'info.main' }
      default: return { bgcolor: 'success.50', color: 'success.main' }
    }
  }

  const renderTabContent = () => {
    switch (activeTab) {
      case 0: // Moteur 108 pts
        return (
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
              <Typography variant="h6" sx={{ color: 'text.primary' }}>
                Controles Moteur d'Audit ({registryControls.filter(c => c.actif).length} / {registryControls.length} actifs)
              </Typography>
              <FormControl size="small" sx={{ minWidth: 200 }}>
                ...
                <Select
                  ...
                  onChange={(e) => setRegistryFilter(e.target.value as number)}
                >
                  <MenuItem value={-1}>Tous les niveaux</MenuItem>
                  {[0,1,2,3,4,5,6,7,8].map((n) => (
                    <MenuItem key={n} value={n}>N{n} - {NIVEAUX_NOMS[n as keyof typeof NIVEAUX_NOMS]}</MenuItem>
                  ))}
                </Select>
              </FormControl>
            </Box>

            <TableContainer component={Paper} sx={{ bgcolor: 'grey.100', '& .MuiTableHead-root': { bgcolor: 'grey.300' } }}>
              <Table size="small">
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Ref</TableCell>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Controle</TableCell>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Niveau</TableCell>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Severite</TableCell>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Phase</TableCell>
                ...
                <TableBody>
                  {filteredRegistryControls.map((ctrl, index) => (
                    <TableRow key={ctrl.ref} sx={{ bgcolor: index % 2 === 0 ? P.primary50 : P.primary100 }}>
                      ...
                      <TableCell>
                        <Typography variant="body2" sx={{ color: 'text.primary', fontWeight: 500 }}>{ctrl.nom}</Typography>
                        {ctrl.description && (
                          <Typography variant="caption" sx={{ color: 'text.secondary' }}>{ctrl.description}</Typography>
                        )}
                      </TableCell>
                      <TableCell>
                        <Chip label={`N${ctrl.niveau}`} size="small" sx={{ bgcolor: 'grey.300', color: 'text.primary', fontWeight: 600 }} />
                      </TableCell>
                      <TableCell>
                        <Chip label={ctrl.severiteDefaut} size="small" sx={severiteColor(ctrl.severiteDefaut)} />
                      </TableCell>
                      <TableCell>
                        <Chip label={ctrl.phase || 'PHASE_1'} size="small" sx={{ bgcolor: 'info.50', color: 'primary.main', fontSize: '0.7rem' }} />
                      </TableCell>
                      <TableCell>
                        <Switch
                          checked={ctrl.actif}
                          size="small"
                          onChange={() => handleToggleRegistryControl(ctrl.ref)}
                        />
                      </TableCell>
                    </TableRow>
                  ))}
      ...
      case 1: // Points de Contrôle Legacy
        return (
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
              <Typography variant="h6" sx={{ color: 'text.primary' }}>
                Regles de Controle SYSCOHADA ({controlPoints.filter(p => p.est_active).length} actives)
              </Typography>
              <Button
                variant="contained"
                startIcon={<AddIcon />}
            ...
            <TableContainer 
              ...
              sx={{ 
                bgcolor: 'grey.100',
                '& .MuiTableHead-root': { bgcolor: 'grey.300' }
              }}
            >
              <Table>
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Code</TableCell>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Règle</TableCell>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Catégorie</TableCell>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Portée</TableCell>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Criticité</TableCell>
                    <TableCell sx={{ color: 'text.primary', fontWeight: 600 }}>Statut</TableCell>
                ...
                <TableBody>
                  {controlPoints.map((point, index) => (
                    ...
                    >
                      <TableCell>
                        <Typography variant="body2" sx={{ fontFamily: 'monospace', color: 'text.primary', fontWeight: 600 }}>
                          {point.code}
                        </Typography>
                      </TableCell>
                      <TableCell>
                  ...
                  <Card sx={{ bgcolor: 'grey.100' }}>
                    <CardContent>
                      <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                        <AIIcon sx={{ color: 'text.primary', mr: 1 }} />
                        <Typography variant="h6" sx={{ color: 'text.primary' }}>
                          {algo.nom}
                        </Typography>
                        <Chip 
                          label={algo.type}
                          size="small"
                          sx={{ ml: 'auto', bgcolor: 'grey.300', color: 'text.primary' }}
                        />
                      </Box>
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Paper sx={{ bgcolor: 'grey.100' }}>
        ...
        >
          <Tab label="Moteur 108 pts" />
          <Tab label="Regles Legacy (64)" />
      ...
      >
        ...
        <DialogActions>
          ...
          <Button variant="contained" sx={{ bgcolor: 'text.primary' }}>

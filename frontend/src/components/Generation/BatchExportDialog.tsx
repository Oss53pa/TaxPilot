...
const BatchExportDialog: React.FC<BatchExportDialogProps> = ({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const result = await generationService.exportBatch(selectedLiasses, exportFormat) as Record<string, any>
      ...
      while (!downloadUrl && attempts < maxAttempts) {
        ...
        const status = await generationService.getBatchExportStatus(batchId) as Record<string, any>
                    ...
                    secondary={
                      <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                        ...
                        <Chip
                          ...
                          size="small"
                          color={
                            liasse.statut === 'TERMINEE' || (liasse.statut as string) === 'VALIDEE'
                              ? 'success'
                              : 'default'
                          }
                        />

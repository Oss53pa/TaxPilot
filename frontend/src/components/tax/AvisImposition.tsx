...
const AvisImposition: React.FC<AvisImpositionProps> = ({
  ...
  const loadAvis = async () => {
    try {
      ...
      const response = await taxService.getDeclarations(params) as Record<string, any>
      ...
      const avisData: AvisDetails[] = (response.results || []).map((declaration: DeclarationFiscale) => {
        ...
        const penalitesPotentielles = joursRestants < 0 ? arrondiFCFA(Math.abs(joursRestants) * montantRestant * 0.001) : 0
          ...
          <Card>
            <CardContent sx={{ textAlign: 'center' }}>
              ...
              <Typography variant="h5" sx={{ fontWeight: 700, mt: 1, color: 'warning.main', fontSize: '1.5rem' }}>
                {formatMontant(stats.total_a_payer)}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
          ...
          <Card>
            <CardContent sx={{ textAlign: 'center' }}>
              ...
              <Typography variant="h5" sx={{ fontWeight: 700, mt: 1, color: 'success.main', fontSize: '1.5rem' }}>
                {formatMontant(stats.total_paye)}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
      ...
      {!loading && avis.length > 0 && (
        <Card>
          <CardHeader
            ...
            subheader={`Restant à payer: ${formatMontant(stats.total_a_payer - stats.total_paye)}`}
          />
          <CardContent sx={{ p: 0 }}>
            <TableContainer>
              <Table>
            ...
            <>
                  ...
                  <List dense>
                    <ListItem>
                      ...
                      <ListItemText
                        ...
                        secondary={`${formatMontant(selectedAvis.montant_total)}`}
                    ...
                    <ListItem>
                      ...
                      <ListItemText
                        ...
                        secondary={`${formatMontant(selectedAvis.montant_paye)}`}
                    ...
                    <ListItem>
                      ...
                      <ListItemText
                        ...
                        secondary={`${formatMontant(selectedAvis.montant_restant)}`}
                    ...
                    {selectedAvis.penalites_potentielles > 0 && (
                      <ListItem>
                        ...
                        <ListItemText
                          ...
                          secondary={`${formatMontant(selectedAvis.penalites_potentielles)}`}
              ...
              {selectedAvis.statut_paiement === 'RETARD' && (
                <Alert severity="error">
                  ...
                  <Typography variant="caption">
                    Des pénalités de {formatMontant(selectedAvis.penalites_potentielles)} s'appliquent
                  </Typography>
                </Alert>
              )}

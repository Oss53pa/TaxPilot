 ...
 * Exercice N calculé depuis la balance importée, N-1 depuis la balance N-1.
 */
...
// ────────── Static row structure (no amounts) ──────────
...
const LIGNES_STRUCTURE: LigneStructure[] = [
  ...
  },
  {
    ...
    isTotal: true,
    level: 0,
    totalOf: ['AC', 'AD', 'AE', 'AF', 'AH', 'AI', 'AJ', 'AK', 'AL', 'AM', 'AO', 'AP'],
  },
  ...
  },
  {
    ...
    isTotal: true,
    level: 0,
    totalOf: ['BB', 'BD', 'BE', 'BF', 'BG', 'BI', 'BJ', 'BK'],
  },
...
const BilanActif: React.FC<BilanActifProps> = ({ modeEdition = false }) => {
  ...
  const lignesActif = useMemo(() => {
    const n1 = getLatestBalanceN1()
    const n1Entries = n1?.entries || []

    // Helper: debit-balance sum from N-1 entries
    const sumDebitN1 = (prefixes: string[]) =>
      Math.round(
        n1Entries
          .filter(e => prefixes.some(p => e.compte.startsWith(p)))
          .reduce(
            (s, e) =>
              s + Math.max(0, (e.solde_debit || e.debit || 0) - (e.solde_credit || e.credit || 0)),
            0,
          ),
      )

    // First pass: compute individual (non-total) row values
    const rows = LIGNES_STRUCTURE.map(row => {
      if (row.comptes) {
        return {
          ...row,
          exerciceN: bal.d(row.comptes),
          exerciceN1: sumDebitN1(row.comptes),
        }
      }
      return { ...row, exerciceN: null as number | null, exerciceN1: null as number | null }
    })

    // Build lookup for total computation
    const rowMap = new Map(rows.map(r => [r.ref, r]))

    // Second pass: compute totals (including DZ which sums AZ, BZ, CZ)
    // We need two passes because DZ depends on AZ/BZ/CZ which are themselves totals.
    // First resolve section totals, then resolve DZ.
    const resolvedRows = rows.map(row => {
      if (row.totalOf && !row.isTotalGeneral) {
        const sumN = row.totalOf.reduce(
          (s, ref) => s + ((rowMap.get(ref) as any)?.exerciceN || 0),
          0,
        )
        const sumN1 = row.totalOf.reduce(
          (s, ref) => s + ((rowMap.get(ref) as any)?.exerciceN1 || 0),
          0,
        )
        const resolved = { ...row, exerciceN: sumN, exerciceN1: sumN1 }
        rowMap.set(row.ref, resolved)
        return resolved
      }
      return row
    })

    // Final pass: resolve DZ (TOTAL GÉNÉRAL) which sums the section totals
    return resolvedRows.map(row => {
      if (row.isTotalGeneral && row.totalOf) {
        const sumN = row.totalOf.reduce(
          (s, ref) => s + ((rowMap.get(ref) as any)?.exerciceN || 0),
          0,
        )
        ...
        return { ...row, exerciceN: sumN, exerciceN1: sumN1 }
      }
      return row
    })
  }, [bal])
  ...
  return (
    <Box>
      ...
      <TableContainer component={Paper} sx={{ border: '2px solid #2563eb' }}>
